				...
				<div className="flex-1 space-y-4">
					<div className="flex items-start justify-between gap-4">
						<div>
							...
							<p className="text-sm text-gray-500">{t("venuesPage.card.clientId", { id: venue.clientId })}</p>
					...
					<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
						<div className="bg-gray-50 rounded-xl px-4 py-3">
							...
							<p className="text-sm font-semibold text-gray-900">{venue.price || t("common.notAvailable")}</p>
						</div>
						<div className="bg-gray-50 rounded-xl px-4 py-3">
							<p className="text-xs text-gray-400">{t("venuesPage.card.availability")}</p>
							<p className="text-sm font-semibold text-gray-900">{availabilityLabel}</p>
						</div>
						<div className="bg-gray-50 rounded-xl px-4 py-3">
							<p className="text-xs text-gray-400">{t("venuesPage.card.discounts")}</p>
							<p className="text-sm font-semibold text-gray-900">{venue.discounts || t("common.notAvailable")}</p>
						</div>
						<div className="bg-gray-50 rounded-xl px-4 py-3">
							<p className="text-xs text-gray-400">{t("venuesPage.card.extras")}</p>
							<p className="text-sm font-semibold text-gray-900">{venue.extras || t("common.notAvailable")}</p>
						</div>
					</div>
				</div>
			</div>
	...
	return (
		<div className="space-y-6">
			...
			<div className="space-y-6">
				...
				{!isLoading && !isError && venues.map((venue) => <VenueCard key={venue._id} venue={venue} />)}
			</div>
		</div>
	);
};
