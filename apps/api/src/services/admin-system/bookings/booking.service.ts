...
export const getBookingsByClientId = async (clientId: number) => {
    return await Booking.find({ clientId: clientId }).where({ isDeleted: false });
};
...
export const getBookingsByCustomerId = async (customerId: number) => {
    return await Booking.find({ customerId: customerId }).where({ isDeleted: false });
};
...
export const deleteBooking = async (id: string) => {
    const booking = await Booking.findByIdAndUpdate(id, { deletedAt: new Date(), isDeleted: true }, { new: true }).where({ isDeleted: false });
